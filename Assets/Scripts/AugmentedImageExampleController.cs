...
namespace GoogleARCore.Examples.AugmentedImage
{
    ...
    public class AugmentedImageExampleController : MonoBehaviour
    {
        ...
        public GameObject ManipulatorExample;
        public List<AugmentedAnimal> AnimalPrefabs;

        public Text t;
        public Text t2;
        public Text t3;
        //public List<Text> augImages;
        /// <summary>
        /// The overlay containing the fit to scan user guide.
        /// </summary>
        public GameObject FitToScanOverlay;

        private Dictionary<int, AugmentedAnimal> m_Visualizers
            = new Dictionary<int, AugmentedAnimal>();
        ...
        public void Update()
        {
            ...
            {
                t.text = image.TrackingMethod.ToString();
                t2.text = image.Name;
                t3.text = "";
                foreach (var augmentedImage in m_TempAugmentedImages)
                {
                    t3.text += augmentedImage.Name + "|";
                }
                // Get visualizer
                m_Visualizers.TryGetValue(image.DatabaseIndex, out var visualizer);

                if (image.TrackingState == TrackingState.Tracking
                    && image.TrackingMethod != AugmentedImageTrackingMethod.LastKnownPose
                    && visualizer == null) 
                {
                    var anchor = image.CreateAnchor(image.CenterPose);
                    visualizer = Instantiate(AnimalPrefabs[image.DatabaseIndex].GetComponent<AugmentedAnimal>(), anchor.transform);
                    visualizer.ManipulatorPrefab = ManipulatorExample;
                    ...
                    && visualizer != null)
                {
                    DisableAugmentedImageObject(image, visualizer);
                }
            }
